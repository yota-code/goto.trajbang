...
class CoordinatedTurn() :
	...
	def __init__(self, psidot_max, phi_max) :
		...
		print(f">>> psidot={self.psidot_max:0.3g}°/sec, phi={self.phi_max:0.3g}° transition={self.vtr:0.3g} kt")

	def psidot_deg(self, v_kt) :
		# compute the psidot limit at high speed
		phi_rad = np.radians(self.phi_max)
		v_ms = 1852 * v_kt / 3600
		return min( np.degrees(self.g * np.tan(phi_rad) / v_ms), self.psidot_max )
	...
	def circle_radius(self, v_kt) :
		v_ms = 1852 * v_kt / 3600
		psidot = np.where(v_kt < self.vtr, self.psidot_max, self.psidot_deg(v_kt))
